...
>(({ id, className, children, config, categories, ...props }, ref) => {
...
export function ChartBars({ children }: { children: React.ReactNode }) {
  return <>{children}</>
}
...
export function ChartBar({ value, name }: ChartBarProps) {
  return <Bar dataKey="value" name={name} fill="hsl(var(--primary))" radius={[4, 4, 0, 0]} />
}
